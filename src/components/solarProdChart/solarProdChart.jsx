...
import jsonData from '../../utils/solar_data_output.json';
...
const SolarProdChart = () => {
  ...
  const prepareData = () => {
    return jsonData.map((entry) => {
      ...
      const formattedDate = entry.date; // Convert 'DD-MM-YYYY' to 'YYYY-MM-DD'
      const formattedTime = entry.time; // Convert 'HH-MM-SS' to 'HH:MM:SS'
      
      // Create a new Date object
      return {
        date: new Date(`${formattedDate}T${formattedTime}`), // Use T for ISO format
        solar_production: entry.solar_production
      };
    });
  };
  ...
  const getChartData = () => {
    return {
      ...
      datasets: [{
        ...
        data: filteredData.map(item => item.solar_production),
